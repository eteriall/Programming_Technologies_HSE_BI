...
"MSWin32-x86-multi-thread-64int-6.1" or
	die "Errno architecture (MSWin32-x86-multi-thread-64int-6.1) does not match executable architecture ($Config{'archname'}-$Config{'osvers'})";
...
our %EXPORT_TAGS = (
    ...
    WINSOCK => [qw(
	WSAEINVALIDPROCTABLE WSAECANCELLED WSAEMFILE WSAELOOP WSAENOTEMPTY
	WSAECONNREFUSED WSAEHOSTDOWN WSAEINVALIDPROVIDER WSAEADDRINUSE
	WSAENOTSOCK WSAEADDRNOTAVAIL WSAEHOSTUNREACH WSAEINTR WSAEFAULT
	WSAENOMORE WSAENOBUFS WSAEALREADY WSAEREFUSED WSAEPROVIDERFAILEDINIT
	WSAETOOMANYREFS WSAEUSERS WSAEDQUOT WSAESOCKTNOSUPPORT WSAEBADF
	WSAECONNRESET WSAENETRESET WSAEINVAL WSAENAMETOOLONG WSAEREMOTE
	WSAENETDOWN WSAETIMEDOUT WSAESTALE WSAEINPROGRESS WSAEPROTOTYPE
	WSAENOPROTOOPT WSAEACCES WSAEWOULDBLOCK WSAECONNABORTED WSAESHUTDOWN
	WSAEDESTADDRREQ WSAEMSGSIZE WSAEDISCON WSAENETUNREACH WSAEPFNOSUPPORT
	WSAEPROTONOSUPPORT WSAEOPNOTSUPP WSAEPROCLIM WSAEAFNOSUPPORT
